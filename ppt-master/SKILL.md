---
name: ppt-master
description: AI-driven multi-format SVG content generation system. Converts source documents
...
> 9. **SVG WRITE METHOD ON WINDOWS — USE write_svg.py** — Every SVG file MUST be written using `scripts/write_svg.py` (explicit UTF-8 via Python `open(..., encoding='utf-8')`). Do NOT rely on the AI's Write tool alone — on Windows it can inconsistently emit GBK/CP936 for CJK-heavy SVG content, corrupting the file. The Write tool is acceptable only as a first attempt; if the per-page UTF-8 verification (Rule 10) fails, you MUST fall back to `write_svg.py`. One-page batch: generate one SVG → verify → write → check → next page. A self-contained per-page Python script that reads the SVG string from a variable and writes it via `write_svg.py` is permitted — this is not the forbidden "batch generator" (looping over page data), it is the verified write-back mechanism.
> 10. **PER-PAGE UTF-8 VERIFY-BEFORE-NEXT** — After writing each SVG page to disk, run this verification command:
> ```bash
> python -c "open(r'<project_path>/svg_output/<page>.svg','r',encoding='utf-8').read(); print('UTF-8 OK')"
> ```
> If it fails (UnicodeDecodeError), the Write tool produced GBK — immediately regenerate the file using `write_svg.py`:
> ```bash
> python -c "import sys; svg='<svg...></svg>'; path=r'<project_path>/svg_output/<page>.svg'; open(path,'w',encoding='utf-8').write(svg); print('Re-written UTF-8:', path)"
> ```
> Then re-verify. Only proceed to the next page after a clean UTF-8 pass. This step runs alongside the quality checker — both must pass before continuing.
> 11. **PER-PAGE QUALITY CHECK** — After each SVG passes UTF-8 verification, run the quality checker for that *single file only*:
> ```bash
> python ${SKILL_DIR}/scripts/svg_quality_checker.py <project_path>/svg_output/<NN>_<page_name>.svg
> ```
> If `[ERROR]` → fix and re-check. If `[WARN]` → evaluate: fix quick wins, note and proceed. Never skip checks or defer to end-of-deck verification.
> 12. **IMAGE PATH SINGLE SOURCE OF TRUTH** — Before writing any SVG that contains `<image>`, read `images/image_manifest.json` (generated by `import-sources`) to get the authoritative filename for each image. Use the exact `<filename>` from the manifest. Never invent or infer image filenames — if the manifest is absent or an image is not listed, use a dashed-border placeholder and surface a warning.
...
| `${SKILL_DIR}/scripts/write_svg.py` | Reliable UTF-8 SVG writer — use this instead of the Write tool on Windows |
...
> ⚠️ **MUST use `--move`** (not copy): all source files — Step 1's generated Markdown, original PDFs / MDs / images — go into `sources/` via `import-sources --move`. After execution they no longer exist at the original location.
>
> ⚠️ **Companion `_files/` directories are auto-moved**: if the source file has a sidecar `<stem>_files/` directory (e.g., a DOCX with extracted images), `import-sources` moves it alongside the archived source so images stay available for propagation.
>
> ⚠️ **Windows compatibility**: always use `python` (not `python3`) for all script invocations in SKILL.md commands. Windows typically has `python` or `py` in PATH, not `python3`.

🚧 **IMAGE VERIFICATION GATE** — after `import-sources` completes, immediately verify images were propagated:

```bash
# List expected images
ls <project_path>/images/
# Verify image_manifest.json exists and is valid UTF-8
python -c "import json; data=json.load(open(r'<project_path>/images/image_manifest.json','r',encoding='utf-8')); print(f'Images in manifest: {len(data)}'); [print(f'  - {it[\"filename\"]}') for it in data]"
```

> If `images/` is empty or `image_manifest.json` is missing: this means DOCX/PPTX embedded images were not extracted. Check whether `sources/<source>_files/` directory exists and contains images — if so, manually copy to `images/`:
> ```bash
> python -c "import shutil,os; src=r'<project_path>/sources/<stem>_files'; dst=r'<project_path>/images'; [shutil.copy(src+'/'+f, dst+'/'+f) for f in os.listdir(src) if f.endswith(('.png','.jpg','.jpeg','.gif','.webp','.bmp','.emf','.wmf'))]; print('Done')"
> ```
> Then re-run the verification command. Proceed to Step 3 only after images are confirmed present.

**✅ Checkpoint — Confirm project structure created successfully, `sources/` contains all source files, `images/` contains all expected image files, converted materials are ready. Proceed to Step 3.**
...
```
- Start it immediately when Executor begins; `svg_output/` may be empty. Editor opens at `http://localhost:5050`.
- **Port conflict detection — MANDATORY before starting**: run `netstat -ano | Select-String ":5050"` first. If any process is already listening on port 5050, kill it before starting:
  ...
  taskkill /PID <PID> /F
  ```
  Multiple servers can bind to the same port on Windows (SO_REUSEADDR), but only the first-started one actually receives requests — the others silently fail. Always clear stale processes before starting a new server.
...
**Image Manifest Read (Mandatory before any image-bearing page)**: before writing any SVG page that contains an `<image>` element, read `images/image_manifest.json` (created by `import-sources`) and use the exact `<filename>` value from it as the `href` path. Common filename patterns: DOCX/PDF-extracted images use underscores (`word_media__image_001.png`), renamed images use simple names (`image1.png`, `video_001.png`). Never guess — always check the manifest.
...
**Visual Construction Phase — Per-Page Verify-Before-Next**: generate SVG pages sequentially, one at a time, in one continuous pass → `<project_path>/svg_output/`. After writing each page to disk, run **both** checks before proceeding to the next page:

> ⚠️ **UTF-8 verification FIRST** — always run this before the quality checker. If the Write tool produced GBK, the quality checker itself will crash with a decode error before it can report anything useful.
> ```bash
> python -c "open(r'<project_path>/svg_output/<page>.svg','r',encoding='utf-8').read(); print('UTF-8 OK')"
> ```
> If `UnicodeDecodeError`: the Write tool emitted GBK. Re-write immediately using this command, then re-verify:
> ```bash
> python -c "import sys; svg=<...SVG content...>; open(r'<project_path>/svg_output/<page>.svg','w',encoding='utf-8').write(svg)"
> ```
> **Only after UTF-8 passes** run the quality checker:
> ```bash
> python ${SKILL_DIR}/scripts/svg_quality_checker.py <project_path>/svg_output/<NN>_<page_name>.svg
> ```
> If the check returns `[ERROR]` → fix the error in that file immediately, re-run the check, only proceed to the next page when it passes `[OK]`.
> If the check returns `[WARN]` → evaluate: if the warning is quick to fix (e.g. a color value not in spec_lock), fix it now; if it's an advisory (e.g. a long text line), note it and proceed.
> **Never skip the per-page check** and "check all at the end" — that pattern leads to cascading errors and full-regeneration loops.
- If a page passes cleanly (`[OK]`, no warnings), proceed immediately to the next page without waiting.

> ⚠️ **On Windows: encoding troubleshooting** — if the quality checker's Chinese output appears as garbled characters in PowerShell, this is a **terminal display issue only** (PowerShell's default code page). The SVG files themselves are written in UTF-8 by the Write tool and are NOT corrupted. Do NOT regenerate files based on garbled output. To verify: run `python -c "open('<project_path>/svg_output/<file>.svg','r',encoding='utf-8').read()"` — if it reads without error, the file is fine. Alternatively, run the checker with output redirected: `python ${SKILL_DIR}/scripts/svg_quality_checker.py <file> 2>&1 | Out-File -Encoding utf8 report.txt`.

> ⚠️ **Image path mismatch** — the most common source of `[ERROR]` on image-bearing pages is a wrong `href` filename. Always verify the actual image filename against `images/image_manifest.json` (or `images/` directory listing) before writing the SVG. Mismatched hrefs produce `Image file not found` errors.
...
**Quality Check Gate (Safety Net)** — runs only after ALL pages have passed their per-page checks (see §B above). This is a full-deck safety net, not the primary gate:
```bash
python ${SKILL_DIR}/scripts/svg_quality_checker.py <project_path>
```
- Any remaining `error` (should be none if per-page gates were followed) MUST be fixed — regenerate only that page, re-check that page only.
...
- **Troubleshooting**: on generation issues (encoding display, image paths, full-regeneration loops, etc.), check `scripts/docs/troubleshooting.md` for known solutions — including how to distinguish terminal garbling from real file corruption on Windows
- **Windows UTF-8 write**: use `scripts/write_svg.py` as the canonical write-back tool on Windows. The AI's Write tool is a first attempt only — always verify with `python -c "open(path,'r',encoding='utf-8').read()"`. If it fails, re-write via write_svg.py or the inline Python command in Rule 10.
